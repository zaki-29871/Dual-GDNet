...
import numpy as np
from LEAStereo.models.genotypes_2d import PRIMITIVES
from LEAStereo.models.genotypes_2d import Genotype
from LEAStereo.models.operations_2d import *
import torch.nn.functional as F
import numpy as np
import pdb
...
class newFeature(nn.Module):
    def __init__(self, network_arch, cell_arch, cell=Cell,
                 fea_step=3, fea_num_layers=6, fea_block_multiplier=4, fea_filter_multiplier=8):
        super(newFeature, self).__init__()
        self.cells = nn.ModuleList()
        self.network_arch = torch.from_numpy(network_arch)
        self.cell_arch = torch.from_numpy(cell_arch)
        self._step = fea_step
        self._num_layers = fea_num_layers
        self._block_multiplier = fea_block_multiplier
        self._filter_multiplier = fea_filter_multiplier
