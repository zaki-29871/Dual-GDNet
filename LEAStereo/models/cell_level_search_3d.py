...
class Cell(nn.Module):
    ...
    def prev_feature_resize(self, prev_feature, mode):
        ...
        return F.interpolate(prev_feature, (feature_size_d, feature_size_h, feature_size_w), mode='trilinear',
                             align_corners=True)
    ...
    def forward(self, s0, s1_down, s1_same, s1_up, n_alphas):
        ...
        all_states = []
        if s0 is not None:
            s0 = F.interpolate(s0, (size_d, size_h, size_w), mode='trilinear', align_corners=True) if (s0.shape[
                                                                                                           3] != size_h) or (
                                                                                                                  s0.shape[
                                                                                                                      4] != size_w) or (
                                                                                                                  s0.shape[
