...
import numpy as np
from LEAStereo.models.genotypes_3d import PRIMITIVES
from LEAStereo.models.genotypes_3d import Genotype
from LEAStereo.models.operations_3d import *
...
class Cell(nn.Module):
    ...
    def forward(self, prev_prev_input, prev_input):
        ...
        if self.downup_sample != 0:
            ...
            s1 = F.interpolate(s1, [feature_size_d, feature_size_h, feature_size_w], mode='trilinear',
                               align_corners=True)
...
class newMatching(nn.Module):
    def __init__(self, network_arch, cell_arch, cell=Cell,
                 mat_step=3, mat_num_layers=12, mat_block_multiplier=4, mat_filter_multiplier=8):
        super(newMatching, self).__init__()
        self.cells = nn.ModuleList()
        self.network_arch = torch.from_numpy(network_arch)
        self.cell_arch = torch.from_numpy(cell_arch)
        self._step = mat_step
        self._num_layers = mat_num_layers
        self._block_multiplier = mat_block_multiplier
        self._filter_multiplier = mat_filter_multiplier
            ...
            if i == 0:
                ...
                if i == 1:
                    ...
                    _cell = cell(self._step, self._block_multiplier,
                                 ...
                                 self.cell_arch, self.network_arch[i],
                                 self._filter_multiplier * filter_param_dict[level], downup_sample)
