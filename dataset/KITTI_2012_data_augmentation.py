...
augmentation_multiplier = 2 * (len(blur_kernels) + 1) * (len(gaussian_noises) + 1)
...
def KITTI_data_augmentation(indexes, data_used, total_data_size, scale_ratios=None):
    count = 1
...
KITTI_data_augmentation(indexes[:int(copy_size * 0.8)], 'training', int(copy_size * 0.8) * augmentation_multiplier)

print('Generating testing data')
KITTI_data_augmentation(indexes[int(copy_size * 0.8):], 'testing', int(copy_size * 0.2)*augmentation_multiplier)